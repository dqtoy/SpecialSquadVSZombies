...
public class TouchController : MonoBehaviour
{
    ...
    public List<GameObject> GetGameObjectAtMousePosition()
    {
        ...
        {
            Ray ray = Master.Gameplay.myCamera.ScreenPointToRay(Input.mousePosition);
        ...
        {
            Ray ray = Master.UIGameplay.myCamera.ScreenPointToRay(Input.mousePosition);
            RaycastHit2D[] hits = Physics2D.RaycastAll(ray.origin, ray.direction, Mathf.Infinity);
            foreach (RaycastHit2D hit in hits)
            {
                if (hit)
        ...
        {
            Ray ray = Master.UIMenu.myCamera.ScreenPointToRay(Input.mousePosition);
            RaycastHit2D[] hits = Physics2D.RaycastAll(ray.origin, ray.direction, Mathf.Infinity);
            foreach (RaycastHit2D hit in hits)
            {
                if (hit)
    ...
    public void GetMousePosition()
    {
        if (Input.mousePosition != null)
        {
            if (Master.Gameplay != null)
            {
                mousePositionGameplay = Master.Gameplay.myCamera.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, Input.mousePosition.z));
            }

            if (Master.UIGameplay != null)
            {
                mousePositionUI = Master.UIGameplay.myCamera.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, Input.mousePosition.z));
            }

            if (Master.UIMenu != null)
            {
                mousePositionUI = Master.UIMenu.myCamera.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, Input.mousePosition.z));
            }
        }
    }
