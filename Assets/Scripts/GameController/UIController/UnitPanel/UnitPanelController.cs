...
public class UnitPanelController : MonoBehaviour
{
    ...
    void AssignObject()
    {
        ...
        foreach (string statsItem in Master.UnitData.listStatsItem)
        {
			...
			if (statsItem == "Damage") {
				upgradeStatsItem.title.text = "伤害";
			}
			else if (statsItem == "Health") {
				upgradeStatsItem.title.text = "血量";
			}
			else if (statsItem == "AttackSpeed") {
				upgradeStatsItem.title.text = "攻击速度";
			}
			else if (statsItem == "Range") {
				upgradeStatsItem.title.text = "攻击范围";
			}
			else if (statsItem == "Critical") {
				upgradeStatsItem.title.text = "伤害范围";
			}
    ...
    private void SetStatsAndUpgradeInfo()
    {
        ...
        {
            UpgradeStats.listUpgradeStatsDetail["Range"].alertLabel.text = "技能不支持升级";
